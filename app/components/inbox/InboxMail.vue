...
function onSubmit() {
  ...
  setTimeout(() => {
    ...
    toast.add({
      ...
      description: 'Tu correo se envio correctamente',
      icon: 'i-lucide-check-circle',
      color: 'success'
    })
  ...
  <UDashboardPanel id="inbox-2">
    ...
    <div class="flex flex-col sm:flex-row justify-between gap-1 p-4 sm:px-6 border-b border-default">
      ...
      <p class="max-sm:pl-16 text-muted text-sm sm:mt-2">
        {{ format(new Date(mail.date), 'dd MMM HH:mm', { locale: es }) }}
      </p>
    </div>
      ...
      <UCard variant="subtle" class="mt-auto" :ui="{ header: 'flex items-center gap-1.5 text-dimmed' }">
        ...
        <form @submit.prevent="onSubmit">
          ...
          <div class="flex items-center justify-between">
            <UTooltip text="Adjuntar archivo">
              <UButton
                color="neutral"
                variant="ghost"
